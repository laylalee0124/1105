...
public class MemberController extends HttpServlet {
	...
	public void init() throws ServletException { // 이니트가 돌아가면서 멤버서비스가 객체형태로 저장됨.
		...
		// ------기존의 과정 자체가 문자열을 프로퍼티즈에서 읽어들여서 class를 통해 객체화 하는과정이라 시간이 오래걸림 // 커넥션 풀을
		// 이용해서 처리속도를 올릴수있지않을까? -------
		...
		// Set<Object>는 .k e t s e t()의 저장유형이므로 걍외우기.
		...
		for (Object obj : set) {
			...
			// properties파일내에 key&value형태로 저장한 class 경로인 value를 가지고 아까 저장해둔 uri정보를
			// Properties객체로 가져옴.
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 요청된 uri정보를 변수에 저장 : 이정보에 따라 처리될 서비스가 결정된다. 어떤uri에 어떤서비스가 요청될지는
		// member-service.properties에 저장되어있음.
		...
		if (path != null) {
			request.getRequestDispatcher(path).forward(request, response); // request, response정보를 path로 forward해주면서
																			// jsp로 보내줌.
		} else {
